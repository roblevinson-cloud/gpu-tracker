...
def build():
    ...
    for req in ("data/tokens_by_model.csv", "data/model_prices.csv",
                "data/tokens_daily.csv"):
        if not os.path.exists(req):
            print(f"[econ] missing {req} - skipping until it exists")
            return
    ...
    pr["blended"] = (ri * pr["prompt_usd_per_m"] +
                     ro * pr["completion_usd_per_m"]) / (ri + ro)
    ...
    df = tok.merge(plong, on=["date", "model"], how="left") \
            .dropna(subset=["blended"])
    if df.empty:
        print("[econ] no overlap between token and price data yet - skipping")
        return
    ...
    ax.plot(ma.index, ma["revenue_usd"] / 1e6, color=TEAL, lw=3,
            label="all tokens")
    ...
    sub(ax, f"Tokens x list price, {ri}:{ro} input:output blend, 7d avg - "
            "gross capacity, not receipts")
    ...
    sub(ax, "Falling line + rising revenue = volume outrunning price cuts")
    save(fig, "data/econ_revper_chart.png")
    print(f"[econ] revenue OK - latest "
          f"${ma['revenue_usd'].iloc[-1] / 1e6:.2f}M/day implied")

    # ---------- Layer 2: cost band vs price ----------
    have_perf = os.path.exists("data/perf_log.csv")
    have_px = os.path.exists("data/pricing_index.csv")
    if have_perf:
        perf = pd.read_csv("data/perf_log.csv",
                           parse_dates=["timestamp_utc"])
        perf["throughput_tps"] = pd.to_numeric(perf["throughput_tps"],
                                               errors="coerce")
        tps = (perf.dropna(subset=["throughput_tps"])
               .set_index("timestamp_utc")["throughput_tps"]
               .resample("D").median())
        ...
        if os.path.exists(gpath):
            ...
            gpu_hr = pd.to_numeric(gd.get("lowest_price"),
                                   errors="coerce") \
                .resample("D").mean().ffill()
        else:
            gpu_hr = pd.Series(dtype=float)
        gpu_hr = gpu_hr.reindex(tps.index).ffill() \
            .fillna(A["fallback_gpu_usd_per_hr"])
        ...
        mid = cost_per_m((A["concurrency_low"] +
                          A["concurrency_high"]) / 2).dropna()
        ...
        if have_px:
            px = pd.read_csv("data/pricing_index.csv",
                             parse_dates=["date"])
            px = px.set_index("date")["avg_completion_usd_per_m"].dropna()
            ax.plot(px.index, px, color=ROSE, lw=3,
                    label="weighted market price")
        ax.set_ylim(bottom=0)
        style(ax, "$ per M output tokens")
        ax.legend(loc="upper left", frameon=False, fontsize=12)
        ax.set_title("Serving cost vs market price", loc="left")
        sub(ax, f"{A['node_gpus']}x {A['cost_gpu_vintage'].upper()} node "
                f"at tracker rental price / measured tok/s x "
                f"{A['concurrency_low']}-{A['concurrency_high']} streams")
        save(fig, "data/econ_margin_chart.png")
        print(f"[econ] margin OK - mid cost ${mid.iloc[-1]:.2f}/M")
